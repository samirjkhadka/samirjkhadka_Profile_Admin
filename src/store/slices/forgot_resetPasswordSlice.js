...
export const forgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const response = await axios.post(
      "https://samirjkhadka-profile-backend.onrender.com/api/v1/user/password/forgot",
      { email },
  ...
  (token, password, confirmPassword) => async (dispatch) => {
    try {
      ...
      const response = await axios.put(
        `https://samirjkhadka-profile-backend.onrender.com/api/v1/user/password/reset/${token}`,
